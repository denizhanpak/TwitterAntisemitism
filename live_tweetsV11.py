...
import ssl
# data to get the status of live/dead tweets
import tweepy
consumer_key =  r'YizeWcw1TkPhvM87b5ITWbEW2'
consumer_secret =  r'D1HK4eZR89nVzjDPi3YocrZn7AKbM4E2cWrSTaBAUZSiWbzvlE'
access_token = r'1291217937551036421-TkxT07Cn84xA24vkAkZgTc1GZxvHhj'
access_token_secret = r'G9spJmYEfNHZkCaVce9WiR5Ugsk8nk7fTlMPawCBVRMzs'
...
def tweet_is_live(ID):
    """
    This method checks if the tweet can be returned or not.
    :param ID: ID of the tweet
    :return: returns 1 if the tweet is live else returns 0
    """
    try:
...
def tweet_is_live1(ID:str, username:str, verbose=False) -> bool:
...
def read_files(directory_path:str, sample_size:int, verbose:bool) -> list :
    ...
    for fileobject in os.listdir(folder): 
        ...
        if verbose:
            ...
            with open(os.fsdecode(folder) + "/" + filename, "r", encoding="utf-8") as f:
...
def writeFile(data:list, name:str, verbose=False):
    #Writing in the file
    with open(name, "w+",  newline='', encoding='utf-8') as outfile:
        for i in data:
            try:
                outfile.write(str(i) + "\n") 
            except Exception as e:
...
if __name__ == '__main__':
    ...
    if output == "placeholder":
        output = str(sample_size) + "sample_new.json"
